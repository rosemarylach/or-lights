...
void setup() {
  ...
  for (int i=0; i<4; i++){
    for (int j=0; j<3; j++) {
     zeroPos[i][j] = String("0.00").toFloat();
    }
  }
  */
...
void timerIsr() {
  ...
  */
  ...
  if (dimVal == clock_tick) {
    ...
    digitalWrite(DIM_CH1, LOW);
    digitalWrite(DIM_CH2, LOW);
    digitalWrite(DIM_CH3, LOW);
    digitalWrite(DIM_CH4, LOW);
    digitalWrite(DIM_CH5, LOW);
    digitalWrite(DIM_CH6, LOW);
    digitalWrite(DIM_CH7, LOW);
    digitalWrite(DIM_CH8, LOW);
  }
}
